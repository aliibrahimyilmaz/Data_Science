...
-- NOT: list_price sütununda NULL değerler olmuş olsaydı en üstte gelecekti ve sonuç hatalı olacaktı. Dikkat etmekte fayda var.
-- NOT: Aynı fiyata sahip ürünler olduğunda hata yapmamak için GROUP BY ya da WINDOW function kullanmalısınız. 
...
-- NOT: Aynı sorguda hem TOP hem de OFFSET kullanılmıyor. Deneyip hata verdiğini görebilirsiniz.
...
-- NOT: Bir değerin başka bir tabloda bulunup bulunmamasını kontrol ediyorsanız ve
-- kontrol ettiğiniz tablodan herhangi veriye ihtiyacınız yoksa EXISTS kullanmanız tavsiye edilir.
-- Eğer kontrol ettiğiniz tablodan bir veriye ihtiyacınız olacaksa o zaman zaten join yapmalısınız.
...
-- NOT: From bloğunda yazılan subquerynin sonucunu sanki veritabanınızda bulunan bir tabloymuş gibi düşünebilirsiniz.
...
-- NOT: Her iki sorguda da aynı sayıda column olması lazım.
...
-- NOT: Fonksiyonun NULL çıkması durumunda simple case ile atama yapılamıyor.
-- Çünkü NULL değeri NULL değerine eşit değildir. Search Case expression kullanılması gerekir.


-- Search Case Expression
-- Yukarıdaki sorguda herbir durumu ayrı ayrı fonksiyonlarda tanımlayın
SELECT	*,	CASE
				WHEN DATEDIFF(DAY, order_date, shipped_date) IS NULL THEN 'tarih bilgisi eksik'
...
-- Yukarıdaki sorgu sonuçlarını INT ve NUMERIC veri dönüşümündeki farklar bakımından inceleyin.
